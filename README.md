...
A single, platform-agnostic JSON codec for Go that optimizes WebAssembly binary size by using zero reflection. It relies on `model.Encodable` and `model.Decodable` for struct encoding/decoding, which is typically generated by `ormc`.
...
Structs MUST implement `model.Encodable` and `model.Decodable` to be supported. This is handled by generating code with `ormc`.
...
func (u *User) DecodeFields(r model.FieldReader) error {
